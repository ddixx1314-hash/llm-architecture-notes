...
**1.** 退化成"单头全维度 Attention + 输出投影"。和第 1 节的纯 Self-Attention **不完全一样**:多了 $W^O$。但表达能力上,$W^O \cdot W^V$ 可以合并成一个矩阵($\mathbb{R}^{d_{\text{model}} \times d_{\text{model}}}$),所以等价于把原 $W^V$ 换成 $W^V W^O$ — 实际上**等价**于一个 $W^V$ 更"灵活"的单头 Attention。

**2.** 每个头里 $q, k \in \mathbb{R}^1$ 是标量。点积 $qk$ 也是标量,softmax 之后注意力分布几乎只取决于 $q$ 和 $k$ 的符号。这丢掉了"在多维空间里测量相似性"的本质 — 一维空间只有"方向相同/相反"两种关系。表达能力崩溃,虽然有 $d_{\text{model}}$ 个头,但每个头都很弱。

**3.** 权衡是**每个头的容量 vs 头数**。$h$ 越大每头 $d_k = d_{\text{model}}/h$ 越小,每个头的"相似性度量"维度不够;$h$ 越小,头之间多样性丢失。实证上 $h = 8 \sim 16$ 是甜点。另外 $h$ 必须能整除 $d_{\text{model}}$,所以常取 2 的小幂次。
