...
⚠️ **$R_k$ 不是任意线性映射,而是 block-diagonal 的旋转矩阵**:
...
& & \ddots & \\
& & & R_k^{(d/2-1)}
\end{bmatrix}, \quad R_k^{(i)} = \begin{bmatrix} \cos(k\omega_i) & \sin(k\omega_i) \\ -\sin(k\omega_i) & \cos(k\omega_i) \end{bmatrix}
$$

也就是说,$d_{\text{model}}$ 维空间被分解成 $d_{\text{model}}/2$ 个**独立的 2 维平面**,每个平面里发生独立的旋转。维度之间不互相耦合。
...
📌 **这一性质会在第 7 节 RoPE(Rotary Position Embedding)中被显式利用 — RoPE 直接把"旋转"作用在 Q、K 上,而不是加到嵌入上。** 同时旋转矩阵的群性质($R_a^T R_b = R_{b-a}$)会让 attention 分数显式依赖相对距离。
...
<div align="center"><img src="images/positional-encoding-heatmap.png" width="92%"></div>

图:正弦/余弦位置编码可视化。**上**:完整热图,横轴是位置 $pos$、纵轴是维度索引 $i$、颜色是编码值。可以清楚看到低维度(图上方)振荡频率高,高维度(图下方)振荡频率低 — 像一组转速不同的"时钟指针"。**下**:抽出几个维度画成曲线,直观看到 dim 0 频率最高(几乎是噪声状),dim 120 频率极低(几乎线性)。每个位置 $pos$ 对应整个 $d_{\text{model}}$ 维向量的一个"快照",由这些不同频率波形在该位置的取值组合而成。
...
**1.** 底数变小,最低频维度的波长也变小。$\lambda_{\max} = 2\pi \cdot 2 \approx 12.57$ — 最慢指针每 13 个 token 就转一圈。位置 0 和位置 13 在所有维度上几乎一样,**位置混淆**。所以底数必须远大于最大序列长度。

**2.** 数学上 sin/cos 对和单个复数 $e^{i\theta} = \cos\theta + i\sin\theta$ 是等价的。但**实际神经网络层(Linear / matmul)只对实数运算**。用复数会引入复杂的实部/虚部混合处理(本质上还是两个实数维度),没有任何节省,反而让框架实现复杂。所以选实数 sin/cos 是工程选择,数学上等价。

**3.** **可以**外推,因为公式是闭式的,任意 $pos$ 都能算。但**效果会显著下降**:
- 模型训练时见过的注意力 pattern 只覆盖 $[0, n_{\text{train}})$ 的相对距离
- 推理时遇到的 $pos$ 落入"训练分布外",虽然 PE 值仍合法但模型没学过这些相位组合
- 最低频维度上,如果 $pos$ 突破训练分布,模型可能"区分不出来"位置 1 和位置 1000(因为指针几乎在同一相位)

这就是为什么 RoPE + 位置插值/外推(NTK-aware scaling, YaRN 等)成为长上下文研究热点。
